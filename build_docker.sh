...
sed 's#PYTORCH_VERSION#'${PYTORCH_VERSION}'#g'   > Dockerfile.cpu

docker build ${EXTRA_ARGS} -t neofung/deep_learning_cpu:latest \
    -t neofung/deep_learning:${VERSION}-py_${PYTHON_VERSION}-tf_${TENSORFLOW_VERSION}-torch_${PYTORCH_VERSION}-cpu -f Dockerfile.cpu  .
